...
exports.createValidationMiddlewares = (tables_object) => {
	...
	tables.forEach( table => {
	  ...
	  custom_routes.forEach( custom_route => {
	  	let [ validation_case, validation_params ] = setVars(``, Object.keys(tables_object[table].routes[custom_route].validation));

		validation_params.forEach((validation_param, index) => validation_case += writeValidationLines(tables_object[table].routes, custom_route, validation_param, validation_params.length, index));
	  ...
	  default_routes.forEach( default_route => {
	    if(isUndefined(tables_object[table].routes[default_route])){
		  ...
		  let [ all_validation_params, validation_default_params, validation_case ] = setVars(Object.keys(tables_object[table].fields), Object.keys(default_values.default_routes[default_route].validation), ``);
			...
			if(vd_default_param.includes("all except")){
			  all_validation_params.forEach( (this_validation_param, j) => {
			  	if(!vd_default_param.includes(this_validation_param)){
			  	  validation_case += writeValidationLines(default_values.default_routes, default_route, this_validation_param, all_validation_params.length, j);
			  	}
			  });
			} else {
			  validation_case += writeValidationLines(default_values.default_routes, default_route, vd_default_param, validation_default_params.length, i);
			}
		  });
...
const writeValidationLines = (routes_object, route_name, validation_param, global_params_length, global_index) => {
  ...
  let [ validation_lines, validation_object, more_lines_condition ] = setVars(``, undefined, false);
